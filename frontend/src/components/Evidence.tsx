...
export function Benchmark() {
  ...
  if (err) return <div className="status-label status-blocked"><Icon name="eye" /><span>{err}</span></div>
  if (!data) return <p className="muted">Reading the scored runs…</p>
    ...
    <>
      ...
      <div className="pair" style={{ marginTop: 18 }}>
        {data.fixture.map((run) => (
          <div className="card" key={run.tag}>
            <span className="status-label status-queued mono">{run.tag}</span>
            <div className={`stat-big ${run.found === run.planted ? 'good' : 'bad'}`}
                 style={{ marginTop: 8 }}>
              {run.found}<span className="of">/{run.planted}</span>
            </div>
            <span className="eyebrow">defects found</span>
                ...
                {run.rows.map((r) => (
                  <tr key={r.criterion} className={r.run ? '' : 'dim'}>
                    ...
                    {r.run ? (
                      ...
                      <td colSpan={4} className="muted">not run</td>
                    )}
                  </tr>
                ))}
              </tbody>
          ...
          <ul style={{ margin: '10px 0 0 20px', fontSize: '0.83rem', lineHeight: 1.7 }}>
            {axeItems.map((i, n) => (
              <li key={n}>
                ...
                <span className="status-label status-queued">best practice, not a success criterion</span>
              </li>
            ))}
          </ul>
        )}
        ...
        <>
          ...
          <div className="pair" style={{ marginTop: 14 }}>
            {data.unknown.map((run) => (
              <div className="card" key={run.key}>
                <span className="status-label status-queued mono">{run.label}</span>
                <div className="stat-big" style={{ marginTop: 8 }}>{run.targets.length}</div>
                <span className="eyebrow">elements named</span>
                <table className="grid" style={{ marginTop: 12 }}>
                  <thead>
                    <tr>
      ...
      <p className="muted" style={{ marginTop: 16 }}>
        ...
        <a href={data.weave} target="_blank" rel="noreferrer">
          Weave <Icon name="external" size={14} />
...
export function Loop() {
  ...
  if (err) return <div className="status-label status-blocked"><Icon name="eye" /><span>{err}</span></div>
  if (!data) return <p className="muted">Reading the loop runs…</p>
  ...
  return (
    <>
      ...
      <div className="pair" style={{ marginTop: 14 }}>
        {arms.map((a) => (
          <div className="card" key={a.tag}>
            <span className="status-label status-queued mono">{a.arm} · lessons {a.lessons}</span>
            <div className="stat-big" style={{ marginTop: 8 }}>{a.mean_attempts ?? '—'}</div>
            <span className="eyebrow">patch attempts per closed finding</span>
            <table className="grid" style={{ marginTop: 12 }}>
              <thead>
                <tr>
                  ...
                  <tr key={r.order}>
                    ...
                    <td>
                      {r.lesson_ids.length === 0 ? (
                        <span className="muted">nothing to retrieve</span>
                      ) : r.trace ? (
                        <a href={r.trace} target="_blank" rel="noreferrer" className="mono">
                          <Icon name="target" size={14} /> {r.lesson_ids.join(', ')}
                        </a>
                      ) : (
